...
func pack_textures_popup() -> void:
	if window != null:
		...
		return
	window = (ResourceLoader.load(WINDOW_SCENE, "PackedScene") as PackedScene).instantiate()
...
func _init_texture_picker(p_parent: Node, p_image_index: int) -> void:
	...
	texture_button.set_script(ResourceLoader.load(DRAG_DROP_SCRIPT, "GDScript") as GDScript)
