...
func start_rename(val: bool = false) -> void:
	...
	data_directory = ""
	if not DirAccess.dir_exists_absolute(dir_name):
		print("An error occurred when trying to access the path: ", data_directory)
		return

	var affected_files: PackedStringArray
	var files: PackedStringArray = DirAccess.get_files_at(dir_name)
	for file_name in files:
		if file_name.match("terrain3d*.res"):
			...
			DirAccess.rename_absolute(dir_name.path_join(file_name), dir_name.path_join(new_name))
