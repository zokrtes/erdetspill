...
func add_tool_button(p_params: Dictionary) -> void:
	...
	button.set_button_icon(ResourceLoader.load(p_params.get("add_icon"), "Texture2D") as Texture2D)
	...
	if p_params.has("sub_text"):
		...
		button2.set_button_icon(ResourceLoader.load(p_params.get("sub_icon", p_params.get("add_icon")), "Texture2D") as Texture2D)
