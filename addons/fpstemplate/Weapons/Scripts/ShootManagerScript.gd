...
func getCameraPOV():
	...
	#Create intersection space to contain possible collisions (areas default OFF — quest NPCs use Area3D).
...
func projectileShot(pointOfCollisionProjectile : Vector3):
	...
	# Create a clean transform with only position and rotation (no scale)
	var cleanTransform = Transform3D.IDENTITY
	cleanTransform.origin = cW.weaponSlot.attackPoint.global_position
	cleanTransform.basis = cW.weaponSlot.attackPoint.global_transform.basis.orthonormalized()  # Remove scale
