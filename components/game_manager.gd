...
# PLAYER VARIABLES
# ============================================
## Cinematic intro on the main level (`IntroSequence`). Turn off in the Inspector on the GameManager autoload for faster iteration.
...
func _play_quest_complete_sound() -> void:
	...
	elif ResourceLoader.exists("res://assets/sfx/hl1-master/sound/buttons/blip1.wav"):
		audio.stream = load("res://assets/sfx/hl1-master/sound/buttons/blip1.wav")
	else:
		print("Quest complete sound aborted: no fallback audio file found")
		audio.queue_free()
		return
	audio.play()
